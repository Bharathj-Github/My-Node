...
exports.login = async (req, res) => {
  ...
  if (!username) {
    res.status(206).json({ message: "UserName Must provide" });
  } else {
    let user = await model.findOne({ username });
    if (!password) {
      if (user === null) {
        res.status(206).json({ message: "userName is not in the database" });
      }else {
      ...
      bcrypt.compare(password,user.password,(err,result)=>{
        if(result){
          ...
          const accessToken = jwt.sign(user_id, process.env.ACCESS_TOKEN_SECRET,{expiresIn: "30s"});
          res.cookie('jwt', `${accessToken}`,{expiresIn: '30s'});
          res.status(200).json({message: "success",accessToken});
        }
        else{
          res.status(206).json({message: "username and password doesn't match"})
        }
      })
    }
  }
